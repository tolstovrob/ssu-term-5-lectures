...
$ g(x_k) = f_k,space forall k in overline(0\, n) $ <raz1:par1:eq1>
...
/ Определение.: Функция $g(x)$, удовлетворяющая условиям (@raz1:par1:eq1) называется *интерполирующей* (интерполяционной, приближающей) функцией. Короче: *интерполянта*.

/ Определение.: Условия (@raz1:par1:eq1) называются *главным условием интерполяции (ГУИ)*.
...
== Интерполяционный\* многочлен в общем виде <raz1:par2:label>
...
$ p_n (x) = a_n x^n + a_(n-1) x^(n-1) + ... + a_1 x + a_0. $ <raz1:par1:eq2>

Потребуем, чтобы (@raz1:par1:eq2) удовлетворял (@raz1:par1:eq1), то есть:
...
Равенства (@raz1:par1:eq3) по своей алгебраической природе представляют собой СЛАУ размерности $(n+1) times (n+1)$ относительно неизвестных коэффициентов интерполяции $a_0, a_1, ..., a_n$. Чтобы её решить, необходимо, чтобы её главный определитель был отличен от нуля:

$
  Delta_(\(#text([@raz1:par1:eq3])\)) = mat(
    delim: "|",
    x_0^n, x_0^(n-1), ..., x_0, 1;
    x_1^n, x_1^(n-1), ..., x_1, 1;
...
Чтобы (@raz1:par1:eq4) был отличен от нуля необходимо, чтобы:
$ forall i != j space x_i != x_j. $ <raz1:par1:eq5>

Условие (@raz1:par1:eq5) называется условием попарной различности узлов интерполяции.

Таким образом, из выше изложенного можем получить следующее: при условии (@raz1:par1:eq5) определитель (@raz1:par1:eq4) отличен от нуля, а значит (@raz1:par1:eq3) имеет единственное решение. Таким образом, решив (@raz1:par1:eq3) каким-либо подходящим численным методом найдём её единственное решение, а именно значение искомых коэффициентов $a_0, a_1, ..., a_n$. В свою очередь, найдя эти коэффициенты и подставив их в исходное представление (@raz1:par1:eq2), мы получим *явную аналитическую форму* для исходного представления интерполянты:

$ P_n (x) = a_n x^n + a_(n-1) x^(n-1) + ... + a_1 x + a_0, $ <raz1:par1:eq6>
...
+ По данным интерполяции $(x_k, y_k), space k = overline(0\, n)$ записать общий вид искомого многочлена (@raz1:par1:eq2);
+ Построить вспомогательную СЛАУ (@raz1:par1:eq3);
+ Решить вспомогательную СЛАУ (@raz1:par1:eq3);
...
Итак, мы установили, что система (@raz1:par1:eq3) имеет единственное решение, а значит, искомая интерполянта вида (@raz1:par1:eq2) будет определена *единственным образом!*

Поскольку способ, описанный в предыдущем параграфе имеет вычислительные ограничения с ростом числа узлов интерполяции, то, зная, что в задаче интерполяции, при условии (@raz1:par1:eq5) существует, и при том единственное решение, будем решать задачу, но иным способом.
...
Теперь покажем, что представление (@raz1:par2:eq2) действительно удовлетворяет ГУИ. Для этого подставим в него в качестве значения аргумента $x$ узел $x_j, space j = overline(0\, n)$:
...
Но отметим, что пока представление (@raz1:par2:eq2) непригодно для нахождения значений интерполянты вне узловых точках (что необходимо для полного разрешения ЗИ).

В этой связи, зная, что ФМЛ в точках $l_k (x)$ $x_0, space x_1, dots, x_(k-1), x_(k+1), ..., x_n$ принимает 0 в силу (@raz1:par2:eq3), применим *основную теорему алгебры*.
...
Также потребуем, чтобы представление (@raz1:par2:eq4) удовлетворяло условию нормировки $l_k (x_k) = 1 space forall k in overline(0\,n)$:
...
Таким образом, осуществляя последовательную подстановку сначала (@raz1:par2:eq5) в (@raz1:par2:eq4), а затем (@raz1:par2:eq4) в (@raz1:par2:eq2), мы получим искомую явную аналитическую *формулу интерполяционного многочлена в форме Лагранжа*:
...
В данном параграфе рассмотрим ещё один способ построения ИМ, существование и единственность которого были изучены в параграфе @raz1:par2:label, при этом данный способ будет лишён недостатков, которым обладали 2 предыдущих способа ЗИ.
...
Снова получаем уравнение. Подставим (@raz1:par4:eq3) в (@raz1:par4:eq2), получим уравнение вида:
...
В этой связи равенство (@raz1:par4:eq5) принимает следующий вид:
...
Таким образом, в условиях сделанного предположения формул (@raz1:par4:eq5) позволила нам получить явное аналиттическое выражение интерполируемой функции. Откажемся от нашего предположения, то есть возьмём произвольные функцию $f(x)$ и многочлен $N_n (x)$:

$ N_n(x) & = f(x_0) \ & + f(x_0, x_1)(x - x_0) \ & + f(x_0, x_1, x_2)(x - x_0)(x - x_1) \ & + ... \ & + f(x_0, ..., x_n)(x - x_0)...(x - x_(n-1)) $ <raz1:par4:eq7>

Непосредственной подстановкой можно убедиться, что алгебраический многочлен (@raz1:par4:eq7) действительно удовлетворяет ГУИ:
...
Для того, чтобы воспользловаться (@raz1:par4:eq7) для нахождения значения интерполянты в неузловой точке, предварительно надо вычислить соответствующие РР. Для удобства, РР записывают в виде следующей таблицы (ТРР). // TODO: переверстать
...
Равенства в системе (@raz1:par5:eq2) образуют СЛАУ относительно $a_i, b_i, c_i, d_i$. Система не имеет единственного решения, недостаёт двух уравнений.
...
По своей алгебраической природе (@raz1:par5:eq3) содержит $(2n - 2)$ уравнения, что вместе с уравнениями (@raz1:par5:eq2) даёт $(2n + 2n - 2) = (4n - 2)$ уравнения. Недостающие два уравнения получим из так называемого _условия нулевой кривизны_ для сплайнов $S_1 (x)$ в точке $x_0$ и $S_n (x)$ в точке $x_n$. Пусть они в пределе станут параллельными оси абсцисс:

$
  cases(S''_1 (x_0) = 0\,, S''_n (x_n) = 0\,) <=> cases(2c_1 = 0\,, 2c_n + 6d_n h = 0.)
$ <raz1:par5:eq4>

Итак, мы получили СЛАУ размерности $4n$, состоящую из уравнений (@raz1:par5:eq2), (@raz1:par5:eq3), (@raz1:par5:eq4). Можно убедиться, что её главный определитель будет отличен от нуля и она будет иметь единственное решение.

Решив её любым методом, получим искомые коэффициенты для каждого сплайна, определяемых по формулам (@raz1:par5:eq1)
